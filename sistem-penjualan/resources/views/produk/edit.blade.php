    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h4><i class="fas fa-edit"></i> Edit Produk: {{ $produk->nama_produk }}</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('produk.update', $produk) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control @error('jenis_produk') is-invalid @enderror" 
                            ...
                            @php
                                $jenisOptions = ['Smartphone Android', 'Smartphone iOS', 'Tablet', 'Laptop', 'Aksesoris'];
                            @endphp
                            @foreach($jenisOptions as $jenis)
                                <option value="{{ $jenis }}" 
                                    {{ old('jenis_produk', $produk->jenis_produk) == $jenis ? 'selected' : '' }}>
                                    {{ $jenis }}
                                </option>
                            @endforeach
                        </select>
